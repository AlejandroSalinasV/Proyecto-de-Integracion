...
class Graficador():
    ...
    def graficar_curvas_FSM(self):
        ...
        cv2.putText(canvas2,"Las esntradas son: "+str(fsm1.tabla_nombres_entradas),(20, 420),cv2.FONT_HERSHEY_SIMPLEX,0.35, (0, 0, 0), 1, cv2.LINE_AA)
        ...
        filename = "g/03_Diagrama_estados_binarios" + ".jpg"
        cv2.imwrite(filename,canvas2)
        graficas.append(filename)
        ...
        for i in range(0,int(len(muestras[muestras_llaves[0]])/2)):
            ...
            fxcoor = calc.punto_medio_entre_dos_coordenadas(flecha_coords[0],flecha_coords[2]) + int(-15+32*math.cos(angle[i]))
            fycoor = calc.punto_medio_entre_dos_coordenadas(flecha_coords[1],flecha_coords[3]) + int(7+18*math.sin(angle[i]))
            ...
            str_bin_entrada_transicion = calc.decimal_a_binario(int(muestras[muestras_llaves[0]][i]),fsm1.bits_salida)
